...
u16 timer2GetPrescaler(void)
{
	...
	// get the current prescaler setting
	return (pgm_read_word(TimerRTCPrescaleFactor+(inb(TCCR2A) & TIMER_PRESCALE_MASK)));
}
#endif
...
u32 timerMsToTics(u16 ms)
{
	...
	u16 prescaleDiv = 1<<(pgm_read_byte(TimerPrescaleFactor+inb(TCCR0A)));
...
void timerPause(unsigned short pause_ms)
{
	...
	// prescaler division = 1<<(pgm_read_byte(TimerPrescaleFactor+inb(TCCR0A)))
